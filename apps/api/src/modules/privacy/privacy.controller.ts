...
export class PrivacyController {
  ...
  @Post(['/api/privacy/requests', '/api/tenants/:tenantId/privacy/requests']) create(@CurrentTenant() tenantId: string, @CurrentUser() user: any, @Body() body: CreateDataSubjectRequestDto) { return this.privacy.createRequest(tenantId, body.phone, body.requestType, body.details, user.id); }
  @Post(['/api/privacy/requests/:id/process', '/api/tenants/:tenantId/privacy/requests/:id/process']) process(@CurrentTenant() tenantId: string, @CurrentUser() user: any, @Param('id') id: string) { return this.privacy.startProcessing(tenantId, id, user.id); }
  @Post(['/api/privacy/requests/:id/reject', '/api/tenants/:tenantId/privacy/requests/:id/reject']) reject(@CurrentTenant() tenantId: string, @CurrentUser() user: any, @Param('id') id: string, @Body() body: RejectDataSubjectRequestDto) { return this.privacy.reject(tenantId, id, body.reason, user.id); }
