...
export class MetricsRetentionService {
  ...
  async getPolicy(tenantId: string): Promise<{ retentionDays: number | null }> {
    const result = await this.db.query('SELECT metrics_retention_days FROM tenants WHERE id = $1', [tenantId]);
    const value = result.rows[0]?.metrics_retention_days;
    return { retentionDays: value === null || value === undefined ? null : Number(value) };
  }
