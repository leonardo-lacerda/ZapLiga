...
import { AnalyticsLearningService } from '../analytics-learning/analytics-learning.service';

@Injectable()
export class DecisionShadowService {
  constructor(private readonly decisions: DecisionRepository, private readonly policies: DecisionPolicyService, @Optional() private readonly analytics?: AnalyticsEventsService, @Optional() private readonly learning?: AnalyticsLearningService) {}
  ...
  async recordBatch(tenantId: string, campaignId: string, candidates: Array<{ lead: any; fifoPosition: number; eligibility?: EligibilityResult }>) {
    ...
    const historical = await this.learning?.shadowSignal(tenantId).catch(() => null);
    const scored = candidates.map((candidate) => ({ ...candidate, score: scoreLead({
      ...
      now: new Date(), policy: policy.weights, policyVersion: policy.version, historicalBoost: historical?.boost,
    }) }));
    ...
    for (const candidate of scored) {
      ...
      void this.analytics?.record({
        ...
        aggregateType: 'lead',
        aggregateId: String(candidate.lead.id),
        campaignId,
        idempotencyKey: `decision.scored:shadow:${campaignId}:${policy.id}:${candidate.lead.id}:${dedupeWindow}`,
        payload: {
          leadId: candidate.lead.id,
          score: candidate.score.score,
          policyVersion: policy.version,
          reasonCodes: [...(candidate.eligibility?.reasonCodes ?? []), ...candidate.score.reasons.map((reason) => reason.code)],
        },
      }).catch(() => undefined);
    }
