import { BadRequestException, Body, Controller, Delete, Get, Param, NotFoundException, Post, Query, UploadedFile, UseGuards, UseInterceptors } from '@nestjs/common';
...
export class LeadsController {
  ...
  async create(@Body() body: CreateLeadDto, @CurrentTenant() tenantId: string, @CurrentUser() user: any) {
    ...
    const lead = await this.db.transaction(async (client) => {
      ...
      const existing = await client.query('SELECT 1 FROM leads WHERE tenant_id = $1 AND phone = $2 LIMIT 1', [tenantId, phone]);
      if (!existing.rows[0]) {
  ...
  async import(@UploadedFile() file: Express.Multer.File, @CurrentTenant() tenantId: string, @CurrentUser() user: any) {
    ...
    const imported = await this.db.transaction(async (client) => {
      await this.planLimits.acquireTenantLock(tenantId, client);
      await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [`lead-quota:${tenantId}`]);
      const existing = uniquePhones.length ? await client.query('SELECT phone FROM leads WHERE tenant_id = $1 AND phone = ANY($2::text[])', [tenantId, uniquePhones]) : { rows: [] as any[] };
      ...
      if (newPhones.length) await this.planLimits.assertCanAddLeads(tenantId, client, newPhones.length);
