...
export function CampaignsPage({ tenantId, folders, sdrs, numbers, featureEnabled = true, decisionFeatureEnabled = false }: CampaignsPageProps) {
  ...
  const loadDecision = useCallback(async () => {
    if (!tenantId || !selected?.id || selected.is_legacy || !decisionFeatureEnabled) {
      setDecisionPolicy(null); setDecisionComparison(null); setDecisionSimulation(null); setDecisionMessage(''); return;
    }
    try {
      const [policy, comparison] = await Promise.all([
        json(`/api/tenants/${tenantId}/campaigns/${selected.id}/decision-policy`),
        json(`/api/tenants/${tenantId}/campaigns/${selected.id}/decision-comparison`),
      ]);
  ...
  const changeDecisionMode = async (mode: string) => {
    ...
    try {
      ...
      setDecisionMessage(mode === 'active' ? 'Fila inteligente ativada para esta campanha.' : mode === 'shadow' ? 'Modo sombra restaurado.' : 'Fila inteligente desligada; a ordem configurada continua valendo.');
    } catch (error) { setDecisionMessage(errorMessage(error)); } finally { setDecisionBusy(false); }
  };
