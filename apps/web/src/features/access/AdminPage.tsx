...
export function AdminPage({ onChanged, onOpenTenant }: { onChanged?: () => Promise<void>; onOpenTenant?: (tenantId: string) => void }) {
  ...
  const [tenantFlags, setTenantFlags] = useState<AnyRow>({});
  ...
  const openTenantDetails = async (tenantId: string) => {
    ...
    try {
      const [detail, numbers, history, flags] = await Promise.all([
  ...
  const deleteTenantUser = async (tenantId: string, member: AnyRow) => {
    if (!window.confirm(`Apagar ${member.name} desta empresa? Ele sai da empresa e, se não pertencer a nenhuma outra, a conta é apagada definitivamente.`)) return;
    setBusy(`member-delete-${member.user_id}`); setMessage(''); setSuccess('');
    try {
      const result: AnyRow = await json(`/api/admin/tenants/${tenantId}/users/${member.user_id}`, { method: 'DELETE', headers: { 'x-tenant-id': tenantId } });
      setSuccess(result?.account === 'deleted' ? 'Usuário removido e conta apagada.' : result?.account === 'blocked' ? 'Usuário removido; a conta foi bloqueada (ela tem registros que não podem ser apagados).' : 'Usuário removido desta empresa; a conta continua ativa nas outras empresas.');
      await load(true);
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); }
    finally { setBusy(''); }
    await openTenantDetails(tenantId);
  ...
  const deleteTenant = async (tenant: AnyRow) => {
    const typed = window.prompt(`APAGAR DEFINITIVAMENTE a empresa ${tenant.name}?\n\nTodos os leads, chamadas, números, métricas e usuários que só pertencem a ela serão apagados. Não há como desfazer (só pelo backup).\n\nPara confirmar, digite o nome exato da empresa:`);
    if (typed === null) return;
    if (typed.trim() !== String(tenant.name).trim()) { setMessage('Nome não confere. Nada foi apagado.'); return; }
    setBusy(`tenant-delete-${tenant.id}`); setMessage(''); setSuccess('');
    try {
      const result: AnyRow = await json(`/api/admin/tenants/${tenant.id}`, { method: 'DELETE', body: JSON.stringify({ confirmName: typed.trim() }) });
      if (selectedTenant?.tenant?.id === tenant.id) setSelectedTenant(null);
      const warn = result?.waxumFailures?.length ? ` Atenção: ${result.waxumFailures.length} sessão(ões) Waxum não foram removidas.` : '';
      setSuccess(`Empresa ${tenant.name} apagada. ${fmt(result?.deletedUsers ?? 0)} conta(s) apagada(s), ${fmt(result?.blockedUsers ?? 0)} bloqueada(s).${warn}`);
      await load(true);
      await onChanged?.();
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); }
    finally { setBusy(''); }
