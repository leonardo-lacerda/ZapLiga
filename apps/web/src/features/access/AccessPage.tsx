...
export function AccessPage({ tenantId, role }: { tenantId: string; role: string }) {
  ...
  return <>
    ...
    <Panel><SectionHeader title="Convidar usuário" description="O convite expira e o usuário criará a própria senha ao aceitar." /><form className="form-row" onSubmit={invite}><input type="email" placeholder="email@empresa.com" value={email} onChange={(event) => setEmail(event.target.value)} required /><select value={inviteRole} onChange={(event) => setInviteRole(event.target.value as 'sdr' | 'leader')}><option value="sdr">SDR</option>{canInviteLeaders && <option value="leader">Líder</option>}</select><Button icon="plus">Enviar convite</Button></form></Panel>
