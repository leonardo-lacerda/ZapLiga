  ...
  return <>
    ...
    <div className="dashboard-grid">
      ...
      {isSdr ? <Panel>
        ...
        <div className="number-status-list">{(status.numbers ?? []).map((number: AnyRow) => <div className="number-status" key={number.id}><div className="number-info"><span className="number-icon"><Icon name="phone" size={14} /></span><div><strong>{number.label}</strong><small>{labelStatus(number.status)} · {number.active_calls}/{number.max_concurrent_calls} chamadas</small></div></div><span className={Number(number.cooldown_remaining_seconds) > 0 ? 'text-warning' : 'text-success'}>{Number(number.cooldown_remaining_seconds) > 0 ? `Cooldown: ${formatSeconds(number.cooldown_remaining_seconds)}` : 'Cooldown: livre'}</span></div>)}{!(status.numbers ?? []).length && <EmptyState title="Nenhum número cadastrado" description="Adicione um número para iniciar a operação." />}</div>
