...
import { formatCallReason, formatCallResult, formatPipelineStage, labelStatus } from '../shared/format';
...
export function DataTable({ rows, columns, actions }: { rows: AnyRow[]; columns: string[]; actions?: (row: AnyRow) => ReactNode }) {
  return <div className="table-container"><div className="table-scroll"><table><thead><tr>{columns.map((column) => <th key={column}>{columnLabels[column] ?? column}</th>)}{actions && <th>Ações</th>}</tr></thead><tbody>{rows.map((row, index) => <tr key={row.id ?? index}>{columns.map((column) => <td key={column}>{['status', 'state'].includes(column) ? <Badge tone={statusTone(row[column])}>{labelStatus(row[column])}</Badge> : column === 'available' ? <Badge tone={row[column] ? 'success' : 'neutral'}>{row[column] ? 'Disponível' : 'Offline'}</Badge> : column === 'duration_seconds' ? formatDuration(row[column]) : ['outcome', 'failure_reason', 'last_failure_reason'].includes(column) ? formatCallReason(row[column]) : column === 'call_result' ? formatCallResult(row[column]) : ['pipeline_stage', 'lead_pipeline_stage'].includes(column) ? formatPipelineStage(row[column]) : String(row[column] ?? '—')}</td>)}{actions && <td><div className="table-actions">{actions(row)}</div></td>}</tr>)}</tbody></table></div>{!rows.length && <EmptyState title="Nada por aqui ainda" description="Os registros aparecerão nesta tabela quando existirem." />}</div>;
}
