...
function AuthenticatedApp() {
  ...
  const connectSdr = async () => {
    ...
    socket.onopen = () => { setConnected(true); socket.send(JSON.stringify({ type: 'identify' })); };
    socket.onmessage = (event) => {
      ...
      if (message.type === 'call_reserved') { setAvailable(false); if (message.lead?.name) setActiveCall({ callId: message.callId, lead: message.lead, number: message.number, phase: 'ringing' }); }
    ...
    socket.onerror = () => setError('Não foi possível conectar o canal do SDR. Tente novamente.');
  ...
  const manualDial = async (event: React.FormEvent) => { event.preventDefault(); if (!connected || !sdrReady) { setError('Conecte-se como SDR antes de iniciar uma ligação.'); return; } if (!available) { setError('Ative "Ficar disponível" antes de iniciar uma ligação.'); return; } if (!manualPhone.trim()) { setError('Informe o telefone que deseja chamar.'); return; } if (!window.confirm(`Ligar agora para ${manualPhone.trim()}?`)) return; setManualCalling(true); setError(''); try { await json('/api/calls/manual', { method: 'POST', body: JSON.stringify({ phone: manualPhone.trim(), name: manualName.trim() || undefined }) }); setManualPhone(''); setManualName(''); await load(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setManualCalling(false); } };
