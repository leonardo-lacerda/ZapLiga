...
import { ApiError, json, wsUrl } from '../services/api';
...
function AuthenticatedApp() {
  ...
  const billing = activeTenant?.billing;
  const [adminWriteMode, setAdminWriteMode] = useState(false);
  const [adminWriteModeBusy, setAdminWriteModeBusy] = useState(false);
  const billingRestricted = !isSdr && billing?.enforcementMode === 'enforce' && billing.mode !== 'full' && !(isSuperAdmin && adminWriteMode && billing.mode === 'read_only');
  useEffect(() => {
    if (!isSuperAdmin) { setAdminWriteMode(false); return; }
    void json('/api/admin/billing/write-mode').then((result: AnyRow) => setAdminWriteMode(Boolean(result?.enabled))).catch(() => undefined);
  }, [isSuperAdmin]);
  const toggleAdminWriteMode = async () => {
    setAdminWriteModeBusy(true);
    try {
      const result: AnyRow = await json('/api/admin/billing/write-mode', { method: 'POST', body: JSON.stringify({ enabled: !adminWriteMode }) });
      setAdminWriteMode(Boolean(result?.enabled));
    } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setAdminWriteModeBusy(false); }
  };
  const sdrBillingRestricted = isSdr && billing?.enforcementMode === 'enforce' && billing.mode !== 'full';
  ...
  const loadOnce = useCallback(async () => {
    try {
      ...
      const loadedSdr = isSdr
        ? await json('/api/me/sdr').catch((e) => e instanceof ApiError && e.status === 404 ? json('/api/me/sdr', { method: 'POST' }) : Promise.reject(e))
        : null;
      const [nextStatus, numbersPage, nextFolders, nextSdrs, callsPage, events] = isSdr
        ? [await json('/api/dialer/sdr-status'), { items: [], total: 0 }, [], loadedSdr ? [loadedSdr] : [], { items: [], total: 0 }, []]
  ...
  const connectSdr = async (reconnecting = false) => {
    ...
    setConnecting(true);
    const ticketResult = await json('/api/auth/ws-ticket', { method: 'POST' }).catch((e) => { if (e instanceof ApiError && e.status === 402) manualDisconnect.current = true; setError(e instanceof Error ? e.message : String(e)); return null; });
  ...
  const toggleDialer = async () => { if (billingRestricted) { setError('Ative uma assinatura para alterar o estado do discador.'); return; } try { await json(status.running ? '/api/dialer/pause' : '/api/dialer/start', { method: 'POST' }); await load(); } catch (e) { setError(String(e)); } };
  ...
  const manualCall = async (id: string, name: string) => { if (sdrBillingRestricted) { setError('Ative a assinatura da organização para iniciar ligações.'); return; } if (!connected || !sdrReady) { setError('Conecte-se como SDR antes de iniciar uma ligação.'); return; } if (postCall) { setError('Finalize o pós-atendimento antes de iniciar uma ligação.'); return; } if (!window.confirm(`Ligar agora para ${name}?`)) return; try { await audio.current.prepare(); setAudioReady(true); await json('/api/calls/manual', { method: 'POST', body: JSON.stringify({ leadId: id }) }); await load(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } };
  const manualDial = async (event: React.FormEvent) => { event.preventDefault(); if (sdrBillingRestricted) { setError('Ative a assinatura da organização para iniciar ligações.'); return; } if (!connected || !sdrReady) { setError('Conecte-se como SDR antes de iniciar uma ligação.'); return; } if (postCall) { setError('Finalize o pós-atendimento antes de iniciar uma ligação.'); return; } if (!manualPhone.trim()) { setError('Informe o telefone que deseja chamar.'); return; } if (!window.confirm(`Ligar agora para ${manualPhone.trim()}?`)) return; setManualCalling(true); setError(''); try { await audio.current.prepare(); setAudioReady(true); await json('/api/calls/manual', { method: 'POST', body: JSON.stringify({ phone: manualPhone.trim(), name: manualName.trim() || undefined }) }); setManualPhone(''); setManualName(''); await load(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setManualCalling(false); } };
  ...
  const navItems: { key: TabKey; label: string; icon: string; group: 'Operação' | 'Análises' | 'Configuração' | 'Conta' | 'Administração' }[] = [
    ...
    ...(!isSdr ? [{ key: 'billing' as TabKey, label: 'Assinatura', icon: 'credit-card', group: 'Configuração' as const }] : []),
  ...
  return <><div className="app-shell">
    <div className={`app-layout${isSdr ? ' is-sdr' : ''}`}><aside className="main-sidebar"><div className="brand"><div className="brand-mark"><Icon name="phone" size={17} /></div><div><strong>Zap<span>Liga</span></strong><small>Operação SDR</small></div></div><AccountSwitcher accounts={savedAccounts} currentName={session?.user.name} currentTenantName={activeTenant?.name} hasActiveCall={Boolean(activeCall || postCall)} onSwitch={handleAccountSwitch} onAdd={addAccount} onRemove={removeSavedAccount} /><nav className="sidebar-nav" aria-label="Navegação principal">{navigationGroups.map((group) => <div className="sidebar-group" key={group.label}><span className="sidebar-label">{group.label}</span>{group.items.map((item) => <button key={item.key} className={`sidebar-item ${tab === item.key ? 'active' : ''}`} onClick={() => setTab(item.key)}><Icon name={item.icon} /><span>{item.label}</span>{item.key === 'calls' && callsTotal > 0 && <em>{callsTotal}</em>}</button>)}</div>)}</nav><div className="sidebar-bottom"><div className="user-card"><span className="avatar">{session?.user.name?.slice(0, 1).toUpperCase()}</span><div><strong>{session?.user.name}</strong><small>{session?.user.email}</small></div></div><Button variant="ghost" onClick={() => void handleLogout()}>Sair da conta</Button></div></aside>
      ...
      <main className="main-content"><div className="module-header"><div className="breadcrumb"><Icon name={isSdr ? 'headset' : 'chart'} size={17} /><strong>{moduleTitle}</strong><span>/</span><span>{activeTenant?.name ?? 'Central de operação'}</span></div><div className="header-actions">{isSdr ? <><span className="sdr-header-user"><strong>{session?.user.name}</strong><small>{connected ? 'Canal conectado' : 'Canal offline'}</small></span><Button variant="ghost" onClick={() => void handleLogout()}>Sair</Button></> : <>{isSuperAdmin && <Button variant={adminWriteMode ? 'danger' : 'ghost'} icon={adminWriteMode ? 'settings' : 'lock'} onClick={() => void toggleAdminWriteMode()} disabled={adminWriteModeBusy} title={adminWriteMode ? 'Admin: edição liberada em empresas somente leitura (expira em 8 h). Clique para voltar ao modo leitura.' : 'Admin: empresas sem assinatura ficam somente leitura. Clique para liberar edição.'}>{adminWriteMode ? 'Admin: edição' : 'Admin: leitura'}</Button>}<DateRangePopover value={dateRange} onChange={setDateRange} /><Button variant={status.running ? 'danger' : 'primary'} icon={status.running ? 'pause' : 'play'} onClick={() => void toggleDialer()} disabled={Boolean(billingRestricted)}>{status.running ? 'Pausar discador' : 'Iniciar discador'}</Button></>}</div></div>
        ...
        {billingRestricted && <div className="alert app-alert billing-readonly" role="status"><Icon name="lock" size={17} /><span>Esta organização está sem uma assinatura ativa. Métricas e dados continuam disponíveis, mas alterações, convites e discagens estão bloqueados.</span><Button variant="ghost" onClick={() => setTab('billing')}>Ver assinatura</Button></div>}
