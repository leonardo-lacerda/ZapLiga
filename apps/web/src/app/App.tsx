...
function AuthenticatedApp() {
  ...
  const disconnect = () => { setAvailable(false); setConnected(false); setSdrReady(false); setMicMuted(false); void audio.current.stop(); control.current?.close(); control.current = undefined; };
  ...
  const connectSdr = async () => {
    ...
    socket.onclose = () => { setConnecting(false); audioCall.current = ''; setAvailable(false); setConnected(false); setSdrReady(false); setMicMuted(false); void audio.current.stop(); setActiveCall(null); };
  };
